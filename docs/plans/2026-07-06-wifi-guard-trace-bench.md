...
- **R3（T3, C3 前半）**: `conn=1 ∧ fsm=2` の文脈で `ev=disc r≠8` が発生する
  エピソードのうち、**ログ内で最初のもののみ**を対象とする（実装相互検証での発見
  2026-07-06: 非自発的切断 = one-shot 発火確定が成り立つのは one-shot が armed の
  初回のみ。boot 行起点の単一エポック保証（§5 前処理）により「ログ内最初 =
  ブート後最初 = armed 確定」が機械的に成立する。2 件目以降の burnt エピソードは
  abort 不要（in-flight なし）が正しい挙動であり R3b の領分 — これにより T3+T3b の
  単一連続キャプチャを両シナリオに掛ける運用が成立する）: 判定は **ev/op 記録ベース**で行う
...
- **R3b（T3b, C3 後半）**: 空成立を認めない**ヒット条件を明示**する（ゲート1第2回
  ...
  **猶予規則（実装相互検証での追加 2026-07-06）**: hit 直後 2 tick 以内の追加
  `ev=disc` は **`r≠8` に限り**同一 AP 喪失の重複配送として猶予する（emitter 生成
  fixture で近接した対の disc 配送が観測されたため）。**`r=8` は猶予窓内でも FAIL**
  — burnt 状態で自発的切断（ASSOC_LEAVE）が発生することはなく、出現は内部
  `disconnect();begin();` の disconnect 部のシグネチャそのものであるため、猶予に
  含めると再アーム検出力を失う。猶予窓後は全 disc が FAIL（従来どおり）。
